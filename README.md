...
4. **After the session exits**, the orchestrator runs the **external gate**: a fresh session spawns the same reviewer agent against the PR, posts the verdict as a `## Gate verdict` PR comment, and prints `GATE:PASS` or `GATE:FAIL` on its last line. PASS → the *script* merges (per autonomy mode). FAIL → the script spawns a focused fix session and re-gates, up to `RALPH_GATE_FIX_ROUNDS` times, then labels `ralph:failed:issue` for a human. An unparsable or missing verdict (a crashed session, or one that narrates status instead of printing the marker) is never treated as a FAIL outright: the script retries with one fresh gate session first — logged and posted as a PR comment — and only spawns a fix session from a verdict it actually parsed as `GATE:FAIL`; a second unparsable verdict ends that round without a fix session.
...
`detect_usage_limit` (one function, `ralph-gh.sh`) first checks an iteration/gate/fix session's raw JSON `is_error` field — a client/transport-set flag the model's own reply can never influence — and only then looks for the CLI's documented usage-limit wording (`You've hit your session limit`, `...weekly limit`, `...Opus limit`, the bare `...limit`, plus the differently-worded usage-credits-exhausted variant `You're out of usage credits`/`Your org is out of usage`) in its `errors` array, `result` text, or stderr. Gating on `is_error` first is deliberate: it's what makes it safe to then read `result` at all. Without it, a session merely *discussing* usage limits (this feature's own review is a real, not hypothetical, example) could say the trigger phrase in an ordinary successful reply without any limit ever being hit — confirmed against the installed CLI, an API-level rejection in `--print --output-format json` replaces `result` with the CLI's own composed message and sets `is_error:true`, with no separate machine-readable "this was a usage limit" field. Deliberately NOT matched: sibling wordings for a permanently-disabled seat or a group limit set to $0 — those are entitlement problems, not transient quota, and requeue-and-retry would spin forever. This wording is **not a stable API** — Anthropic can reword it without notice — so the match is best-effort: anything that doesn't match falls straight through to today's existing "no parsable result" handling, never a guess.
...
- **Default** (`RALPH_WAIT_FOR_RESET=0`): the run exits cleanly, and `last-run.md`'s exit reason records the best-effort reset description ralph-gh could parse out of the session's own output (`usage limit — <description>`, or `usage limit — reset time unknown (not stated in session output)` when none was present).
- **`RALPH_WAIT_FOR_RESET=1`**: instead of exiting, ralph-gh sleeps `RALPH_USAGE_WAIT_SECONDS` (default 1800) and resumes the loop — unless the iteration session already emitted a terminal `<promise>` (`QUEUE_EMPTY`/`SYSTEMIC_FAIL`/`CASCADE_FAIL`/`HALT`) before a *later* gate/fix session hit the limit, in which case that signal wins and the run stops instead of silently resuming past it. This is a bounded wait, not a precise sleep-until-reset — the reset time, when a session states one at all, is prose in a locale/format ralph-gh doesn't try to parse into a schedule. If the limit hasn't actually lifted yet, the next attempt hits the same detection and waits again; each round is still bounded by `RALPH_USAGE_WAIT_SECONDS` and by `--max-iterations` overall.
...
- **Deterministic (bash):** which PRs enter the pipeline (same-repo branches whose issue is `ralph:needs-review` — fork PRs are excluded by design), verdict parsing (an unparsable verdict is never treated as a PASS — a single one gets one retry with a fresh session, a second unparsable verdict is a FAIL, and so is any verdict left behind by a session the orchestrator had to kill at timeout, which is never retried), the yolo allowlist check (fails closed if the diff cannot be fetched), the merge itself, session timeouts.
